...
missing=()
for command in packer ansible-playbook python3; do
  command -v "$command" >/dev/null 2>&1 || missing+=("$command")
done
if ((${#missing[@]})); then
  printf 'validate-local: missing required tools: %s\n' "${missing[*]}" >&2
  printf 'Run make -C image init after installing Packer, Ansible, and Python 3.\n' >&2
  exit 127
fi
...
PYTHONPYCACHEPREFIX="${TMPDIR:-/tmp}/rtpengine-ami-pycache" \
  python3 -m compileall -q -f ansible/roles/rtpengine_ami/files tests
